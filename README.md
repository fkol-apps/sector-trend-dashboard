...
- **PER**: 参考値として上部に出します。**時価総額 ÷ 純利益（直近12ヶ月）**で算出。
  赤字の銘柄は「赤字」と表示します。**銘柄の選定には使っていません**（スコアはあくまで値動きベース）。
...
GitHub の定期実行は**混雑時に数十分〜数時間遅れることがあり、まれに実行されない**ことがあります
（実際に2時間以上遅れた例があります）。取りこぼしを拾う保険として、
**JST 10:00 にもう一度**同じ処理を走らせています。内容が変わらなければコミットは作られません。

### 画面の更新タイミング

フロントは読み込み時に JSON を取得しますが、それだけだと開いたままのアプリが古いままになります。
そこで次のときに自動で取り直します。

- **アプリ／タブが再び表示されたとき**（スマホでホーム画面から戻ってきた場合を含む）
- 表示したまま **10分ごと**
- オフラインから復帰したとき

短時間に何度も取りに行かないよう、**3分以内の再取得はしません**。
ヘッダーの「最終更新」には `（2時間前）` のように経過時間を併記し、
30時間以上古い場合は警告色にします。`⟳` ボタンでいつでも手動で取り直せます。
詳細モーダルを開いている間は再取得しません（表示が入れ替わって邪魔になるため）。
...
| PERの算出方法 | yfinanceの `trailingPE` ではなく `時価総額 ÷ 純利益` で自前計算 | 日本株では株価とEPSの株式分割調整が食い違い、`trailingPE` が実態とかけ離れる（例: 日本製鉄209倍 → 実際は12.5倍）。時価総額と純利益は1株あたりではない絶対額なので分割の影響を受けない |
| 予想PERの非表示 | 実績PERのみ表示 | 予想PERは `株価 ÷ 予想EPS` で、上と同じ分割調整の問題を受ける（日本製鉄1.2倍、ニトリ3.6倍など）。絶対額での代替計算ができないため載せない |
| PERを取る銘柄 | 掲載銘柄（各市場80前後）のみ | yfinanceの `info` は1銘柄1リクエスト。ユニバース全体（約500銘柄）に広げると取得時間もレート制限リスクも跳ね上がる |
