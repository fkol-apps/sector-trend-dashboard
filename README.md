...
セルには銘柄名とリターンだけを置いています。

- **セルの色 = その時間軸のリターン**。上昇＝青、下落＝オレンジ、**濃さ＝騰落率の大きさ**。
  濃さが最大になる騰落率は時間軸ごとに変えています（長期 +100% / 中期 +40% / 短期 +20%）。
- 1600×900 で **全11セクターが1画面**（6枚×2行）に収まります。1265px幅で4枚×3行。
- セルにカーソルを合わせるとティッカー・スコア・市場全体順位がツールチップで出ます。
  クリックすると詳細モーダル（1年チャート＋スコア内訳）。
- 上位3銘柄に満たない枠は、埋めずに破線の空セルにします。

> 掲載されるのは各セクターの上位3銘柄なので、リターンは**ほぼ常にプラス**です
> （実データでは99タイル中マイナスは0〜数件）。そのため色は「上がったか下がったか」より
> 「**どれくらい強く上がっているか**」の濃淡として機能します。
